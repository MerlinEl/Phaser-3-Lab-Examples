...
`;
  ...
  create() {
    ...
    this.renderTexture = this.add.renderTexture(100, 100, 400, 400);
    const baseShader = new Phaser.Display.BaseShader("BufferShader", this.fragmentShader);
    this.active_pipeline = this.add.shader(baseShader, 0, 0, screenWidth, screenHeight);

    // Apply the shader to the render texture
    this.renderTexture.setPipeline(this.active_pipeline);
   
  }
  update(){
        // V každém cyklu aktualizuj uniformní proměnné shaderu s aktuální pozicí kontejneru
    if (this.active_shader && this.renderTexture) {
